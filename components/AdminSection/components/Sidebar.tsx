  ...
  return (
    <aside className={`relative flex flex-col bg-[#0a0a0a] border-r border-neutral-800 transition-all duration-200 ${collapsed ? "w-14" : "w-56"}`}>
      ...
      <div className={`flex items-center gap-3 px-3 py-4 border-b border-neutral-800 ${collapsed ? "justify-center" : ""}`}>
        <div className="flex-shrink-0 w-8 h-8 bg-[#DC2626] rounded flex items-center justify-center">
          <Wrench className="w-4 h-4 text-white" />
        </div>
        {!collapsed && (
          <div>
            <div className="text-white text-xs font-black uppercase tracking-wider leading-tight">Ace Auto</div>
            <div className="text-neutral-500 text-[10px] font-mono uppercase tracking-widest">Repair &amp; Tires</div>
          </div>
        )}
      </div>
          ...
          return (
            <button
              ...
              className={`relative w-full flex items-center gap-3 px-3 py-2.5 text-left transition-colors ${
                active ? "bg-[#DC2626]/10 text-[#DC2626]" : "text-neutral-400 hover:text-white hover:bg-white/5"
              } ${collapsed ? "justify-center" : ""}`}
            >
              {active && <span className="absolute left-0 top-1 bottom-1 w-0.5 bg-[#DC2626] rounded-r" />}
              <Icon className="w-4 h-4 flex-shrink-0" />
              {!collapsed && <span className="text-sm font-medium">{label}</span>}
              {id === "notifications" && notificationCount > 0 && (
                <span className={`ml-auto text-[10px] font-bold bg-[#DC2626] text-white rounded-full px-1.5 py-0.5 ${collapsed ? "absolute top-1 right-1 px-1" : ""}`}>
                  {notificationCount}
                </span>
              )}
            </button>
